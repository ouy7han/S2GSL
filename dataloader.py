...
class ABSA_Dataset(Dataset):
    def __init__(self, args, file_name, vocab, tokenizer,flag):
        ...
        fin=open(file_name+'.sentic','rb')
        idx2graph_s=pickle.load(fin)
        fin.close()

        fin_word_list=open(file_name+'.sentic_word_list','rb')
        word_sentic_list=pickle.load(fin_word_list)
        fin_word_list.close()
        ...
        self.data = self.process(raw_data, vocab, args, tokenizer,idx2graph_s,word_sentic_list,all_dep_info,dependency_type_dict)
    ...
    def process(self, data, vocab, args, tokenizer,idx2graph_s,word_sentic_list,all_dep_info,dependency_type_dict):
        ...
        for d,dep_info in zip(data,all_dep_info):
            ...
            sentic_word_list = word_sentic_list[graph_id]
...
def ABSA_collate_fn(batch):
    ...
    label_list_,sentic_graph_,sentic_word_list_,dep_type_matrix_,dependency_type_dict_) = batch
    ...
    sentic_graph = np.zeros((batch_size, max_lens, max_lens), dtype=np.float32)

    for idx in range(batch_size):
        mlen = sentic_graph_[idx].shape[0]
        sentic_graph[idx, :mlen, :mlen] = sentic_graph_[idx]

    sentic_graph = torch.FloatTensor(sentic_graph)


    sentic_word_list = np.zeros((batch_size, max_lens), dtype=np.float32)
    for idx in range(batch_size):
        mlen = sentic_word_list_[idx].shape[0]
        sentic_word_list[idx, :mlen] = sentic_word_list_[idx]

    sentic_word_list = torch.FloatTensor(sentic_word_list)
    ...
    def get_adj_with_value_matrix(dep_type_matrix):
        final_dep_type_matrix=np.zeros((batch_size, max_lens, max_lens), dtype=np.int)
        for idx in range(batch_size):
