...
def evaluate(model, dataloader, args, vocab):
    ...
    for step, batch in enumerate(dataloader):
        ...
        with torch.no_grad():
            ...
            logits, span_loss, loss_root,adaptive_loss = model(inputs)
            #logits, span_loss, loss_root = model(inputs)
...
def train(args, vocab, tokenizer, train_dataloader, valid_dataloader, test_dataloader, model, optimizer):
    ...
    for epoch in range(1, args.num_epoch + 1):
        ...
        for i, batch in enumerate(train_dataloader):
            ...
            #logits, span_loss, loss_root = model(inputs)
            loss = F.cross_entropy(logits, label, reduction='mean')
            loss = loss + args.alpha * span_loss + args.beta * loss_root+args.gamma *adaptive_loss
            #loss = loss + args.alpha * span_loss + args.beta * loss_root
            loss.backward()
            optimizer.step()
