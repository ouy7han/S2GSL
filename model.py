...
class S2GSL(nn.Module):
    ...
    def forward(self, inputs):
        ...
        con_spans, \
        sentic_graph, sentic_word_list, dep_type_matrix = inputs

        # input
        initial_input = (
        length[map_AS], bert_length[map_AS], word_mapback[map_AS], aspect_token,aspect_indi, bert_sequence, bert_segments_ids,
        adj[map_AS], con_spans, sentic_graph[map_AS], sentic_word_list[map_AS], dep_type_matrix[map_AS])



        final_feature, loss_root, span_loss,adaptive_loss = self.dual_graph_learning_module(initial_input)  # 完全模型
        #final_feature, loss_root, span_loss = self.dual_graph_learning_module(initial_input)  # 完全模型
        hiddens = final_feature

        # aspect-level
        logits = self.classifier(self.dropout(hiddens))

        return logits, span_loss, loss_root,adaptive_loss
...
class Dual_graph_learning(nn.Module):
    ...
    def forward(self, inputs):
        ...
        length, bert_lengths, word_mapback, aspect_token,mask, bert_sequence, bert_segments_ids, adj, con_spans, sentic_graph, sentic_word_list, dep_type_matrix = inputs
        ...
        context, attn, d, d0, loss_root = self.latent_tree_att(bert_out, key_padding_mask, mask, root_mask,
                                                                sentic_graph=None, dep_type_adj=dep_type_adj,
                                                                is_multi_head=True)
        ...
        adaptive_loss=loss_gate*different_loss+(1-loss_gate)*same_loss
        # ---------------------------------------------------------------------------------------------------------------------------
        ...
        # semantic<--syntactic
        sem_fusion_syn_graph_out, _ = self.sem_fusion_syn(sem_graph_out, syn_graph_out, syn_graph_out,
                                                          key_padding_mask)
        # syntactic<--semantic
        syn_fusion_sem_graph_out, _ = self.syn_fusion_sem(syn_graph_out, sem_graph_out, sem_graph_out,
                                                          key_padding_mask)
        ...
        if self.data_name is not 'laptop':
        ...
        representation = torch.cat(
            (a1_rate * sem_fusion_syn_graph_out, a2_rate * syn_fusion_sem_graph_out, a3_rate * sem_concat_syn_graph_out),
            dim=-1)
        as_features = representation

        return as_features, span_loss, loss_root,adaptive_loss
