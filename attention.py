...
def _getMatrixTree_multi(scores,root,is_multi_head):
    ...
    if is_multi_head is True:
        L = torch.sum(A, 2)  # 在维度为1的数，把它们加起来
    else:
        L = torch.sum(A, 1)
...
class StructuredAttention(nn.Module):
    ...
    def forward(self,x,mask=None,roots_label=None,root_mask=None,sentic_graph=None,dep_type_adj=None,is_multi_head=None):
        ...
        if sentic_graph is not None:
            scores = scores + sentic_graph
        ...
        return context,attn,d,d0,loss_root
